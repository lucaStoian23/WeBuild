...
public class Util extends DriverManager {
    ...
    public static void takeScreenShot()  {
         ...
         /**
         String[] name = Util.scenarioName.split(" ");
         TakesScreenshot scrShot = ((TakesScreenshot) driver);
         screenFile = scrShot.getScreenshotAs(OutputType.FILE);
         String path = filePath + name[1]+ "\\" + name[0]+"_"+stepName+".jpg";
         File destFile = new File(path);

         FileUtils.copyFile(screenFile, destFile);

        screenshot = Files.readAllBytes(Paths.get(path));
          */
    }
    ...
    public static void GoToPage(WebDriver d, String url){
        d.get(url);
